...
export default function TopMenu() {
  ...
  return (
    <header className={`fixed inset-x-0 top-0 z-50 transition-colors duration-300 ${
        isScrolled
          ? 'bg-gray-900/80 md:bg-gray-900/80 md:backdrop-blur border-b border-white/10'
          : 'bg-transparent'
      }`}>
      <nav aria-label='Global' className='flex items-center justify-between p-6 lg:px-8' >
        ...
        >
            <Link 
            href={navigationHeading.logo.href}
            className={navigationStyle.logoLink}>
                <span className='sr-only'>{navigationHeading.logo.copy}</span>
                <img
                    alt={navigationHeading.logo.copy}
                    src={navigationHeading.logo.img}
        ...
        <div className='hidden lg:flex lg:gap-x-12'>
            {navigation.map((item) => (
              <Link 
                key={item.name} 
                href={item.href} 
                className={navigationStyle.links}>
                {item.name}
              </Link>
            ))}
        </div>
        <div className='hidden lg:flex lg:flex-1 lg:justify-end'>
            <Link 
              href={navigationHeading.phone.href}
              className={navigationStyle.links}
            >
            {navigationHeading.phone.copy}
            </Link>
        </div>
      </nav>
              ...
              <div className='space-y-2 py-6'>
                {navigation.map((item) => (
                  <Link
                    ...
                    href={item.href}
                    onClick={closeMobileMenu}
                    className={navigationStyle.linksMob}
                  >
                    {item.name}
                  </Link>
                ))}
